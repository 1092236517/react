...
export default class extends BaseViewStore {
    ...
    getList = async(param) => {
        ...
        try {
            const res = await getAgentLaborList({...{...searchValue, ...page}, SpTyp: 3});
            this.view.dataSource = (res.Data.RecordList || []).map((item, index) => ({
                ...item,
                Alladdress: 
                (res.Data.RecordList[index].ProvinceCode && res.Data.RecordList[index].CityCode && res.Data.RecordList[index].AreaCode) 
                ? 
                [res.Data.RecordList[index].ProvinceCode, res.Data.RecordList[index].CityCode, res.Data.RecordList[index].AreaCode] : []
            }));
    ...
    handleSave = async(values) => {
        ...
        let {SpTyp, SpId, SpShortName, CtctName, CtctMobile, BankName, BankCardNo, BankAccountName} = values;
        // 获取地址列表中的数据
        let ProvinceCode = values.Alladdress[0];
        let CityCode = values.Alladdress[1];
        let AreaCode = values.Alladdress[2];
        let param = {SpTyp, SpId, SpShortName, CtctName, CtctMobile, BankName, BankCardNo, BankAccountName, ProvinceCode, CityCode, AreaCode};
        try {
            await editAgentLabor(param);
    ...
    // 同步数据
    @action
    syncAgentLabor= async() => {
        this.view.modalLoading = true;
        try {
            window._czc.push(['_trackEvent', '中介基础数据', '保存同步', '中介基础数据_N非结算']);
            await syncAgentLabor({SpTyp: 3});
