...
class SearchForm extends Component {
    startQuery = (e) => {
        ...
        this.props.form.validateFields((err, fieldsValue) => {
            ...
            const { startQuery, resetPageCurrent } = this.props.noWeekBillLeakOutStore;
            resetPageCurrent();
            startQuery();
            window._czc.push(['_trackEvent', '不发薪.周薪查漏', '查询', '不发薪周薪查漏_Y结算']);
        });
    }
...
SearchForm = Form.create({
    mapPropsToFields: props => {
        const { view: { searchValue } } = props.noWeekBillLeakOutStore;
        return createFormField(searchValue);
    },
    onValuesChange: (props, changedValues, allValues) => {
        props.noWeekBillLeakOutStore.handleFormValuesChange(allValues);
    }
})(SearchForm);
