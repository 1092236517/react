...
export default class extends PureComponent {
    ...
    exportBill = async () => {
        ...
        window._czc.push(['_trackEvent', '盈利报表(旧)', '导出选中对账单', '盈利报表(旧)_Y结算']);
        const reqParam = {
            AuditSts: 2,
            EntId: undefined,
            InvoiceTyp: -9999,
    ...
    render() {
        ...
        const { exportProfitOld } = this.props;
        const { expDataList } = this.state;

        return (
            <div className='mb-16'>
                {authority(exportX)(<Button type='primary' onClick={() => { exportProfitOld(); window._czc.push(['_trackEvent', '盈利报表(旧)', '导出', '盈利报表(旧)_Y结算']); }}>导出</Button>)}
