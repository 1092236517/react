...
export default class extends PureComponent {
    ...
    operData = (type, sts) => {
        ...
        window._czc.push(['_trackEvent', '平台返费账单', messageStr, '平台返费账单_N非结算']);
        // Modal.confirm({
        //     title: '信息',
        //     content: `是否将状态修改为【${messageStr}】`,
    ...
    auditBillOpt = (type) => {
        ...
        window._czc.push(['_trackEvent', '平台返费账单', type === 1 ? '待审核' : '等待', '平台返费账单_N非结算']);
    ...
    addMark = () => {
        ...
        window._czc.push(['_trackEvent', '平台返费账单', '点击添加备注按钮', '平台返费账单_N非结算']);
    }
    saveMark = () => {
        ...
        window._czc.push(['_trackEvent', '平台返费账单', '保存添加备注', '平台返费账单_N非结算']);
    }
    cancelMark = () => {
        this.setState({
            markVisible: false
    ...
    render() {
        const { exportX, auditPassX, auditNotPassX, hasSendLaborX, refusePayX, successPayX, waitX, unAuditX } = resId.returnFee.bill;
        const { exportRecord, auditBillOpt } = this.props;
        const { TextArea } = Input;
        let { type, sts, description, contentOfRemarks } = this.state;
        return (
            <div className='mb-16'>
                {authority(exportX)(<Button type='primary' onClick={()=>{exportRecord();window._czc.push(['_trackEvent', '平台返费账单', '导出', '平台返费账单_N非结算']);}}>导出</Button>)}
